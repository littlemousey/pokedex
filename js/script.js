...
function pokemonDataToDOM(pokemonList) {
  ...
  for (let pokemon of pokemonList) {
    ...
    div.innerHTML = `${pokemonStartNo} <span onclick="showPokemon('${
      pokemon.url
    }')">${prettifyPokemonName(
      pokemon.name
    )}<img src="${pokemonSprites}${pokemonStartNo}.png"/></span>`;
...
function showPokemon(pokemonUrl) {
  fetch(`${pokemonUrl}`)
    ...
    .then(function(singlePokemon) {
      ...
      let pokemonInfo = `<div class="nes-container is-rounded">
			<p class="title">${pokemon.name}</p>
			<div>
			<span><i class="${pokemon.typeIcon}"></i> ${pokemon.type}</span>
			<div id="description"></div>
			<img id="pokemonImage" alt="${pokemon.name}"/>
			<audio id="pokemonCry" src="${
        pokemon.cry
  ...
  var searchPokemon = document
    ...
    .addEventListener("keyup", function() {
      ...
      for (let pokemon of filterPokemon) {
        let div = document.createElement("div");
        div.setAttribute("class", "select-pokemon");
        div.innerHTML = `${pokemon.id} <span onclick="showPokemon('${
          pokemon.url
        }')">${prettifyPokemonName(pokemon.name)}<img src="${pokemonSprites}${
          pokemon.id
        }.png"/></span>`;
        listDom.appendChild(div);
      }
...
function Pokemon(pokemon) {
  ...
  this.typeIcon = determineTypeIcon(this.type[0]);
